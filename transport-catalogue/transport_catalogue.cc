...
void TransportCatalogue::AddStopInternal(const std::string_view name,
                                         ...
                                         bool is_consistent) {
  stops_.emplace_back(Stop{std::string{name}, coordinates, {}, is_consistent});
  stopname_to_stop_[stops_.back().name] = &stops_.back();
}

void TransportCatalogue::AddStop(const std::string_view name,
                                 Coordinates coordinates, bool is_consistent) {
  if (stopname_to_stop_.count(name) == 0) {
    AddStopInternal(name, coordinates, is_consistent);
    return;
  }
...
RouteInfo TransportCatalogue::GetRouteInfo(std::string_view name) {
  if (routename_to_route_.count(name) == 0) {
    return {name, {}, {}, {}, {}};
  }
  Route* route = routename_to_route_.at(name);
  // distance calculation
  double g_distance = 0.;
  size_t r_distance = 0.;
  for (size_t i = 1; i < route->stops.size(); ++i) {
    g_distance += ComputeDistance(route->stops[i - 1]->coordinates,
